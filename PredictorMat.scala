...
REPORT
     ...
     */
    def backwardElim (cols: Set [Int], index_q: Int = index_aic, first: Int = 1): (Int, PredictorMat) =
    {
        var j_mx   = -1                                                  // best column, so far
        var mod_mx = null.asInstanceOf [PredictorMat]                    // best model, so far
        var fit_mx = noDouble                                            // best fit, so far
...
//first add. watch carefully
 rSq(1)    = if (cross) Fit.qofVector (mod_j.fit, mod_j.crossValidate ())   // use new model, mod_j, with cross
                      else       Fit.qofVector (mod_j.fit, null)                     // use new model, mod_j, no cross

		
        println (s"==> forwardSel: add (#1) variable $j, qof = ${mod_j.fit(index_q)}")

	val (h, mod_h) = forwardSel (cols) 		//add 2nd most predictive variable
	cols += h

//second add
 rSq(2)    = if (cross) Fit.qofVector (mod_h.fit, mod_h.crossValidate ())   // use new model, mod_j, with cross
                      else       Fit.qofVector (mod_h.fit, null)                     // use new model, mod_j, no cross

       println (s"==> forwardSel: add (#2) variable $h, qof = ${mod_h.fit(index_q)}")


var mod_temp = mod_h       
var temp = h
var tempfit = mod_h.fit
var added = false
...
break
		...
		if(mod_x.fit(index_q) > tempfit(index_q)) {
	temp = x;
			mod_temp = mod_x;
			tempfit = mod_x.fit
			added = true
	...
	break
		...
		if(mod_y.fit(index_q) > tempfit(index_q)) {
	temp = y;
			mod_temp = mod_y;
			tempfit = mod_y.fit
} else{
...
if(mod_x.fit(index_q) > tempfit(index_q) && mod_x.fit(index_q) > mod_y.fit(index_q)) {
	temp = x;
			mod_temp = mod_x;
			tempfit = mod_x.fit
			added = true
} else if(mod_y.fit(index_q) > tempfit(index_q) && mod_y.fit(index_q) > mod_x.fit(index_q)) {
	temp = y;
			mod_temp = mod_y;
			tempfit = mod_y.fit
} else{
