...
{
        var rSq  = new MatrixD (x.dim2 - 1, 3)                           // R^2, R^2 Bar, R^2 cv
	val cols = Set (0)                                               // start with x_0 in model
	val (j, mod_j) = forwardSel (cols) 			//add most predictive variable
	cols += j

//first add. watch carefully
 rSq(1)    = if (cross) Fit.qofVector (mod_j.fitA(0).fit, mod_j.crossValidate ())   // use new model, mod_j, with cross
                      else       Fit.qofVector (mod_j.fitA(0).fit, null)                     // use new model, mod_j, no cross

		
        println (s"==> forwardSel: add (#1) variable $j, qof = ${mod_j.fitA(0).fit(index_q)}")

	val (h, mod_h) = forwardSel (cols) 		//add 2nd most predictive variable
	cols += h

//second add
 rSq(2)    = if (cross) Fit.qofVector (mod_h.fitA(0).fit, mod_h.crossValidate ())   // use new model, mod_j, with cross
                      else       Fit.qofVector (mod_h.fitA(0).fit, null)                     // use new model, mod_j, no cross


       println (s"==> forwardSel: add (#2) variable $h, qof = ${mod_h.fitA(0).fit(index_q)}")


var mod_temp = mod_h       
var temp = h
var tempfit = mod_h.fitA(0).fit
var added = false
 breakable { for (l <- 3 until x.dim2 - 1) {
            val (x, mod_x) = forwardSel (cols)                           // add most predictive variable
	val(y, mod_y) = backwardElim(cols)		        // remove least predictive variable

	println("Add:")
	if(mod_x != null) {
		println(mod_x.fitA(0).fit(index_q))
	}
	println("Eliminate:")
	if(mod_y != null) {
		println(mod_y.fitA(0).fit(index_q))
	}
...
break
		...
		if(mod_x.fitA(0).fit(index_q) > tempfit(index_q)) {
	temp = x;
			mod_temp = mod_x;
			tempfit = mod_x.fitA(0).fit
			added = true
} else{
	break
}
} else if(x == -1 && y != -1) {
		if(mod_y.fitA(0).fit(index_q) > tempfit(index_q)) {
	temp = y;
			mod_temp = mod_y;
			tempfit = mod_y.fitA(0).fit
} else{
	break
}
} else{
//println("HERE!!!")
//println(mod_x.fit(index_q))
//println(mod_y.fit(index_q))
//println(tempfit(index_q))
if(mod_x.fitA(0).fit(index_q) > tempfit(index_q) && mod_x.fitA(0).fit(index_q) > mod_y.fitA(0).fit(index_q)) {
	temp = x;
			mod_temp = mod_x;
			tempfit = mod_x.fitA(0).fit
			added = true
} else if(mod_y.fitA(0).fit(index_q) > tempfit(index_q) && mod_y.fitA(0).fit(index_q) > mod_x.fitA(0).fit(index_q)) {
	temp = y;
			mod_temp = mod_y;
			tempfit = mod_y.fitA(0).fit
} else{
	break
            ...
            cols     += temp                                           // add variable x_j
            //val completefit = tempfit
            rSq(l)    = if (cross) Fit.qofVector (tempfit, mod_temp.crossValidate ())   // use new model, mod_j, with cross
                        else       Fit.qofVector (tempfit, null)                     // use new model, mod_j, no cross
            if (DEBUG && added) {
                val k = cols.size - 1
                println (s"==> forwardSel: add (#$k) variable $temp, qof = ${tempfit(index_q)}")
            }  else if (DEBUG && !added) {
		                val k = cols.size + 1
               		 println (s"==> backwardElim: remove (#$k) variable $temp, qof = ${tempfit(index_q)}")
}
        }} // breakable for
